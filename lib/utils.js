...
/**
 * Create an `AST` from the given `js`.  Will throw any errors.
 *
 * @api private
 * @param {String} js
 * @param {Function} cb
 */
exports.astSync = function (js) {
  try {
...
exports.uglify = function (js, opts, cb) {
  ...
  function handleAST(err, ast) {
    ...
    process.nextTick(function () {
      cb(null, opts.strings ? exports.strings(str) : str);
    });
  }
...
/**
 * Synchronously uglify the given `js` with `opts`
 *
 * @api private
 * @param {String} js
 * @param {Object} [opts]
 */
exports.uglifySync = function (js, opts) {
  opts = opts || {};
  var ast = exports.astSync(js);
  ast.figure_out_scope();
  ast.mangle_names();
  ast = exports.compress(ast, opts.compressor);
