...
/**
 * Checks if there is exclude option and throws an error if its not valid
 */
function checkExcludes(excludes){
  if (excludes)
    if(!Array.isArray(excludes) || !excludes.length){
      throw new TypeError('Invalid excludes array');
    }
}
 ...
 * @return {Object}
 */
obfuscator.options = function (files, root, entry, strings,excludes) {
...
obfuscator.concatenate = function (options, cb) {
  ...
  built.push(__require.build(options.excludes));
