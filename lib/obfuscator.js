...
/**
 * Synchronously obfuscate and concatenate a NodeJS _"package"_
 * because corporate says so.
 *
 * @api public
 ...
 * @return {String}
 */
obfuscator.sync = function (options) {
  if (!options.files || !options.root || !options.entry) {
    throw new TypeError('Invalid options');
  }

  var code = obfuscator.concatenateSync(options);
  return utils.uglifySync(code, options);
};
...
/**
 * Synchronously register a `file` in location to `root`
 *
 * @api private
 * @param {String} root
 * @param {String} file
 */
obfuscator.registerSync = function (root, file) {
...
/**
 * Synchronously concatenate a list of files for pre-obfuscation
 *
 * @api private
 * @param {Object} options
 */
obfuscator.concatenateSync = function (options) {
  ...
  for (var i = 0, len = files.length; i < len; i++) {
    built.push(obfuscator.registerSync(options.root, files[i]));
  }
